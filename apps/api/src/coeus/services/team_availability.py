...
class TeamAvailabilityService:
    ...
    def calendar(
        ...
        return tuple(
            ...
            if start <= parse_iso_date(entry.entry_date) <= end
        )
    ...
    def availability(self, team: OrgTeam, entry_date: str) -> TeamAvailability:
        ...
        busy = on_leave | on_task | assigned
    ...
    def _calendar_statuses(
        ...
        # Entries are ordered by (date, created_at): the latest wins per user.
        for entry in self._teams.list_entries(team.team_id):
            if entry.entry_date == entry_date and entry.user_id in members:
