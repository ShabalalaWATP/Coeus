...
def _with_board_column(statement: str) -> str:
    """Substitute the shared board-column expression into a fixed statement.

    The placeholder and its replacement are both module constants, so no value
    from a request ever reaches the statement text.
    """
    return statement.replace("@board_column@", _COLUMN)


DETAIL = _with_board_column("""
WITH projected AS (
...
AGGREGATES = _with_board_column("""
WITH projected AS (
 SELECT ownership.owning_unit_id,unit.name unit_name,ownership.target_date,
        aggregate.updated_at ticket_updated_at,@board_column@ board_column
 FROM team_task_ownership ownership
 JOIN coeus_ticket_aggregates aggregate ON aggregate.ticket_id=ownership.ticket_id
 JOIN organisation_units unit ON unit.unit_id=ownership.owning_unit_id
 WHERE ownership.owning_unit_id=ANY(CAST(:unit_ids AS uuid[]))
