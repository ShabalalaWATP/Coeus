...
async def test_ticket_update_failure_rolls_back_the_published_product(
    ...
    async with AsyncClient(
        ...
        original = mutations.save_authorised_if_current_with_confirmation

        def fail_save(*_args: object, **_kwargs: object) -> TicketRecord:
            raise RuntimeError("simulated release persistence failure")

        monkeypatch.setattr(mutations, "save_authorised_if_current_with_confirmation", fail_save)
        ...
        monkeypatch.setattr(mutations, "save_authorised_if_current_with_confirmation", original)
