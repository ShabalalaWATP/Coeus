...
if (!databaseUrl) throw new Error("COEUS_PLAYWRIGHT_DATABASE_URL is required");
if (!objectStoragePath) throw new Error("COEUS_PLAYWRIGHT_OBJECT_STORAGE_PATH is required");

export default defineConfig({
  testDir: "./tests/e2e-postgres",
  testMatch: "zz-sprint24-*.spec.ts",
  timeout: 300_000,
  ...
  webServer: [
    {
      ...
      env: {
        ...
        COEUS_ORGANISATION_DEMO_SEED_ENABLED: "true",
        COEUS_ORGANISATION_MODE: "management",
        COEUS_PERSISTENCE_PROVIDER: "postgres",
        COEUS_SEED_DEMO_CONTENT: "false",
        COEUS_TICKET_MAX_RETAINED_PER_PRINCIPAL: "50",
  ...
  projects: [{ name: "chromium-sprint24-postgres", use: { ...devices["Desktop Chrome"] } }],
});
