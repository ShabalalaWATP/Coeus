...
| Broken or malicious diagram syntax silently corrupts security documentation.                                    | Backend CI parses every tracked Mermaid block with pinned development dependencies in parse-only JSDOM without configured subresource loading and requires Atlas blocks to use allowlisted stable types plus accessible metadata; it does not render or execute diagram links. This is not an OS-level network sandbox, so repository review remains required for dependency changes, meaning and GitHub rendering. |
...
## Air-gapped packaging review (2026-07-23)

The [air-gapped deployment runbook](../runbooks/air-gapped-deployment.md) was
re-reviewed under the
[documentation accuracy specification](../specs/documentation-and-deployment-accuracy.md).

| Threat                                                                                            | Mitigation                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Ignored or untracked working-copy content enters an image labelled with a reviewed commit.        | Packaging exports the exact commit with `git archive` and uses only the exported tree as the Docker context. Docker context exclusions provide a secondary guard for local data, temporary paths and Terraform private state.                                                                                         |
| A source-tree inventory is presented as the SBOM for a transferred image.                         | Syft scans each saved Docker archive separately, preserving the API and web runtime and base-image inventories. The transfer manifest binds both image archives and their SBOMs by SHA-256.                                                                                                                           |
| A failed native scanner, hidden file or nested linked path escapes the release evidence boundary. | PowerShell native-command failures stop the procedure, Gitleaks uses directory mode for the archive export, all hidden and nested release entries are enumerated, reparse points are rejected, and canonical relative paths are hashed recursively. Static security-policy assertions protect these runbook controls. |
