...
| A crashed or competing workflow worker duplicates or loses an external side effect.                                      | Side-effect intents use unique versioned outbox rows. Claims are bounded, skip-locked, leased and worker-fenced; failures retry with a fixed attempt ceiling before dead-lettering. Handlers must use the event ID for idempotency.                    |
...
| A draft read is used to manufacture a new workflow relationship.                                                         | Relationship creation cannot use its prospective projection as authority. External analyst links accept only currently visible published products; creator-readable or projected drafts are denied before ticket and audience persistence. |
