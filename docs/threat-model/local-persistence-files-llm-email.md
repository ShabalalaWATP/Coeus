...
| Object storage failure leaves searchable metadata for missing asset bytes. | Upload rolls back the just-created product record and returns `asset_storage_failed` when local asset bytes cannot be persisted. |
