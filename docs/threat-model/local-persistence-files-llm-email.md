...
| A ticket shadow silently diverges during relational migration.                                                          | Legacy namespace writes and versioned per-ticket shadow rows commit in one PostgreSQL transaction. Canonical hashes are compared on load, disagreement fails closed, and `legacy` is an explicit rollback mode. Each material shadow version has one uniquely keyed outbox event. |
